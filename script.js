...
function render() {
    ...
    ROWS.forEach(function(row) {
        ...
        rowProds.forEach(function(p, index) {
            ...
            const isInActiveOrder = ACTIVE_ORDER.some(function(item) { return item.sku.toLowerCase() === p.sku.toLowerCase(); });
...
function selectSuggestion(sku, boxId, isDB) { 
    ...
    if(isDB) {
        ...
        // Auto scroll dependiendo de la vista activa
        ...
        if (p.hasPO) {
            ...
            if (p.current <= (p.min * 1.5)) {
                newRows += '<tr>' +
                    ...
                    '<td style="font-size:0.8rem; color:var(--muted);">Sol: ' + (p.poDate || '-') + '<br>Lleg: ' + (p.poArrival || '-') + '</td>' +
                '</tr>';
            }
        }
    });
...
function filterDB(q) {
    ...
    PRODUCTS.filter(function(p) { return (p.sku && String(p.sku).toLowerCase().includes(String(q).toLowerCase())) || (p.name && String(p.name).toLowerCase().includes(String(q).toLowerCase())); }).forEach(function(p) {
        ...
        html += '<tr style="' + rowStyle + '">' +
            ...
            '<td><span style="font-family:\'DM Mono\', monospace; color:var(--muted);">' + p.sku + '</span></td>' +
            '<td style="font-weight:500;">' + p.name + '</td>' +
            '<td style="text-align:center;"><input type="number" value="' + displayStock + '" oninput="DB_CHANGES[\'' + p.sku + '\'] = parseInt(this.value) || 0" style="margin:0; font-weight:bold;"></td>' +
            '<td>' + p.min + '</td><td>' + (p.max || 0) + '</td><td>' + statusLabel + '</td>' +
            '<td><button class="btn btn-secondary" style="padding:6px 12px; font-size:0.75rem;" onclick="openProductModal(\'' + p.sku + '\')">Editar</button></td>' +
        '</tr>';
    });
    b.innerHTML = html;
}
...
function openHistoryModal() {
    ...
    if(HISTORY_LOG.length === 0) { 
        html = '<tr><td colspan="6" style="text-align:center; padding:20px; color:var(--muted);">No hay movimientos registrados.</td></tr>'; 
    } else {
        HISTORY_LOG.forEach(function(log) {
            const isPos = String(log.change).startsWith('+');
            const color = isPos ? 'var(--ok)' : 'var(--danger)';
            html += '<tr>' +
                '<td style="color:var(--muted); font-size:0.8rem;">' + log.date + '</td>' +
                '<td><span style="font-family:\'DM Mono\', monospace; color:var(--accent);">' + log.sku + '</span></td>' +
                '<td style="font-weight:500;">' + log.name + '</td>' +
                '<td><b style="color:' + color + '; font-size:1.1rem;">' + log.change + '</b></td>' +
                '<td style="font-size:0.8rem; color:var(--muted);">' + log.user + '</td>' +
                '<td style="color:var(--text);"><i>' + log.reason + '</i></td>' +
            '</tr>';
        });
    }
    b.innerHTML = html;
...
function toggleSapSection() { 
    const section = document.getElementById('sapImportSection'); 
...
function processSapPaste() {
    ...
    lines.forEach(function(line) {
        ...
        if (cols.length >= 2) {
            ...
            const qtyStr = cols[2] ? cols[2].trim() : ''; 
...
function processOrderPaste() {
    ...
    lines.forEach(function(line) {
        ...
        if (columns.length >= 2) {
            ...
            if (sku && requestedQty > 0) {
                ACTIVE_ORDER.push({ sku: sku, name: pastedName, requested: requestedQty, picked: requestedQty, completed: false });
            }
        }
    });
    ...
    if (ACTIVE_ORDER.length > 0) {
        ...
        renderOrderPrepTable();
        sync(); 
        alert("¡Pedido cargado! " + ACTIVE_ORDER.length + " productos resaltados en azul en los racks.");
    }
}
...
function finalizeOrder() {
    ...
    ACTIVE_ORDER.forEach(function(item) {
        ...
        if (pIdx >= 0) {
            ...
            if(descontado > 0) {
                logMovement(PRODUCTS[pIdx].sku, PRODUCTS[pIdx].name, -descontado, "Despacho Pedido (Solicitado: " + item.requested + ")");
                countDespachados++;
            }
        }
    });
...
function openRowModal(id) { const r = (id && typeof id === 'string') ? ROWS.find(function(x) { return x.id === id; }) : {id:'', name:'', sizeM:15}; document.getElementById('rId').value = r.id; document.getElementById('rName').value = r.name; document.getElementById('rSize').value = r.sizeM; document.getElementById('btnDelRow').style.display = id ? 'block' : 'none'; document.getElementById('rowModal').classList.add('open'); }
function saveRow() { const id = document.getElementById('rId').value; const data = { id: id || 'R'+Date.now(), name: document.getElementById('rName').value, sizeM: parseFloat(document.getElementById('rSize').value) || 15 }; if(id) ROWS[ROWS.findIndex(function(x) { return x.id===id; })] = data; else ROWS.push(data); sync(); closeModals(); }
    ...
    if (horaActual >= 8) {
        ...
        if (ultimoEnvio !== hoyStr) {
            ...
            criticos.forEach(function(p) { 
                filas += '<tr>' +
                    '<td style="padding:14px; border-bottom:1px solid #38352f; color:#d4af37; font-weight:bold;">' + p.sku + '</td>' +
                    '<td style="padding:14px; border-bottom:1px solid #38352f; color:#f0ede6;">' + p.name + '</td>' +
                    '<td style="padding:14px; border-bottom:1px solid #38352f; color:#ef4444; text-align:center; font-weight:bold;">' + p.current + '</td>' +
                    '<td style="padding:14px; border-bottom:1px solid #38352f; color:#a39c93; text-align:center;">' + p.min + '</td>' +
                '</tr>'; 
            });

            const estructuraCompletaHTML = '<div style="background:#0d0c0b; color:#f0ede6; font-family:sans-serif; padding:45px; max-width:650px; margin:auto; border:1px solid #38352f; border-radius:16px;">' +
                '<h2 style="color:#d4af37; border-bottom:1px solid #38352f; padding-bottom:15px; text-transform:uppercase; letter-spacing:1px; margin-top:0;">Alerta de Reposición — Concha y Toro</h2>' +
                '<p style="color:#a39c93; font-size:15px;">Reporte automático diario de productos bajo el stock mínimo:</p>' +
                '<table style="width:100%; border-collapse:collapse; margin-top:25px;">' +
                    '<thead><tr style="background:#1a1916; color:#a39c93; font-size:13px; text-transform:uppercase; letter-spacing:0.5px;">' +
                        '<th style="padding:14px; text-align:left; border-bottom:2px solid #38352f;">SKU</th>' +
                        '<th style="padding:14px; text-align:left; border-bottom:2px solid #38352f;">Producto</th>' +
                        '<th style="padding:14px; text-align:center; border-bottom:2px solid #38352f;">Stock</th>' +
                        '<th style="padding:14px; text-align:center; border-bottom:2px solid #38352f;">Mín.</th>' +
                    '</tr></thead>' +
                    '<tbody style="font-size:14px;">' + filas + '</tbody>' +
                '</table></div>';
...
/* ===============================================================
   NUEVA FUNCIÓN: VISTA AÉREA INTERACTIVA (MAPA 2D)
   =============================================================== */
...
function toggleViewMode() {
    ...
    if (currentViewMode === 'racks') {
        ...
        whWrap.style.display = 'flex'; // Default is flex in our app
...
function changeActiveWarehouse(whId) {
...
function renderMap() {
    ...
    whZones.forEach(function(zone) {
        ...
        zEl.ondblclick = function(e) { e.stopPropagation(); openZoneModal(zone.id); }; // Doble clic para editar
        canvas.appendChild(zEl);
    });

    // Separar filas asignadas a esta bodega de las no asignadas (globales)
    const assignedRows = [];
    ...
    // Dibujar Filas Asignadas
    assignedRows.forEach(function(row) {
        const rEl = document.createElement('div');
        rEl.className = 'map-entity-row' + (row.rotation === 90 ? ' rotated' : '');
        rEl.id = 'map-row-' + row.id;
        
        const rWidthPx = row.sizeM * scale;
        const rDepthPx = 1.2 * scale; // Profundidad estándar de rack (1.2m)
        
        if (row.rotation === 90) {
            rEl.style.width = rDepthPx + 'px';
            rEl.style.height = rWidthPx + 'px';
        } else {
        ...
        rowProds.forEach(function(p) {
            ...
            if (row.rotation === 90) { pEl.style.height = pWidthPx + 'px'; } 
            else { pEl.style.width = pWidthPx + 'px'; }

            // Highlighting
            const isInActiveOrder = ACTIVE_ORDER.some(function(item) { return item.sku.toLowerCase() === p.sku.toLowerCase(); });
            if (currentH === p.sku) { pEl.style.boxShadow = '0 0 0 2px var(--bg), 0 0 10px var(--accent)'; pEl.style.zIndex = 10; }
            if (isInActiveOrder) { pEl.style.animation = 'pulseBlue 1.5s infinite'; pEl.style.zIndex = 10; }

            // Tooltip
            let tooltipText = 'SKU: ' + p.sku + '\nProducto: ' + p.name + '\nStock: ' + p.current;
            pEl.setAttribute('data-tooltip', tooltipText);
            
            pEl.onmousedown = function(e) { e.stopPropagation(); }; // Evita arrastrar fila si clickeas producto
            pEl.onclick = function() { openProductModal(p.sku); };
        ...
        rEl.ondblclick = function(e) {
            if (e.shiftKey) { row.whId = null; row.x = 0; row.y = 0; sync(); } // Quitar del mapa
            else { row.rotation = row.rotation === 90 ? 0 : 90; sync(); } // Rotar
        };

        canvas.appendChild(rEl);
    });

    // Renderizar panel lateral (Filas sin asignar)
    ...
    if (unassignedRows.length === 0) {
        ...
        unassignedRows.forEach(function(ur) {
            uHtml += '<div class="unassigned-row-card">' +
                '<b>' + ur.name + '</b>' +
                '<button class="btn btn-secondary" style="padding:4px 8px; font-size:0.7rem;" onclick="assignRowToMap(\'' + ur.id + '\')">Al Plano ➡️</button>' +
            '</div>';
        });
    }
...
// DRAG AND DROP NATIVO DEL LIENZO
function initMapDrag(e, type, id) {
    if (e.button !== 0) return; // Solo clic izquierdo
    e.stopPropagation();
    draggingMapItem = { type: type, id: id };
    
    const target = document.getElementById('map-' + type + '-' + id);
...
function onMapDrop(e) {
    ...
    // Convertir de px a metros según escala y redondear levemente para "snap" sutil (0.1m)
    ...
    sync(); // Guarda en FB y re-renderiza
}
...
function deleteWarehouse() {
    ...
    if(ROWS.some(function(r) { return r.whId === id; })) return alert("Vacía las filas de esta bodega primero (Shift+DobleClic en el plano).");
    if(confirm("¿Eliminar Bodega? Se perderán sus pasillos.")) {
...
function saveZone() {
    ...
    const data = { 
        ...
        x: 0, y: 0 // Inician en 0, luego se arrastran
    };
    
    // Si ya existía, preservar coordenadas
    const exist = ZONES.find(function(x) { return x.id === id; });
    if(exist) { data.x = exist.x; data.y = exist.y; }
